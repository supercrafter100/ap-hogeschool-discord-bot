...
import { ButtonInteraction, type ClientEvents } from 'discord.js';
...
export default class EventHandler extends InteractionHandler<EventCollector> {
    ...
    protected async onLoadFile(path: string) {
        let event: Partial<Event<never>> = {};

        try {
            const mod = await import(pathToFileURL(path).href);
            const Ctor = (mod as any).default ?? mod; // default export or CJS export
            event = new Ctor(this.client);
        } catch (e) {
