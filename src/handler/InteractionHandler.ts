...
export default abstract class InteractionHandler<
    CollectType extends NonNullable<unknown>
> {
    ...
    protected abstract onLoadFile(path: string): unknown;

    //	Handlers

    public abstract handle(...args: unknown[]): Promise<unknown>;
...
export class InteractionRejected extends Error {
    public static is(error: Error): error is InteractionRejected {
